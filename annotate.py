...
def annotateImagesInDirectory(rcnn, directory_path, label):
    for fileName in tqdm(os.listdir(directory_path)):
        if fileName.endswith(".jpg") or fileName.endswith(".jpeg") or fileName.endswith(".png") or fileName.endswith(".tif") or fileName.endswith(".tiff"):
        	# load image
            '''if os.path.exists((directory_path+"/"+fileName).replace(".png",".json")):
                print("found annotation for file %s"%fileName)
                continue'''
            ...
            if class_names.index(label) in result['class_ids']:
                ...
                #print("Annotating...")
                if (args.displayMaskedImages == True):
                    display_instances(img, result['rois'], result['masks'], result['class_ids'],
                                  class_names, class_names.index(label), result['scores'])
                if args.displayMaskedImages==True:
                    keep_or_discard=input("Do you want to save this annotation? (l+Enter:discard/Enter:save)")  
                else:
                    keep_or_discard= ''
                while keep_or_discard not in ['','l']:
                    keep_or_discard=input("Do you want to save this annotation? (l+Enter:discard/Enter:save)")
                if keep_or_discard=='':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--displayMaskedImages', default=False, type=lambda x: (str(x).lower() == 'true'), required=False,help='Display the masked images.')                    
